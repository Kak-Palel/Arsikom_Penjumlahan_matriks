...
.data
    ...
    A db 20 dup (?)                                                 ; Matrix A, maximum size 20
    B db 20 dup (?)                                                 ; Matrix B, maximum size 20
    C db 20 dup (?)                                                 ; Matrix C to store the result
    newline db 13, 10, '$'                                          ; Newline characters for printing

.code
; Subroutine to input matrix A
input_A proc
    mov cx, n               ; Set loop counter to the number of elements
...
add_matrices_loop:
    ...
    inc si                  ; Increment index
    loop add_matrices_loop  ; Loop until CX is 0
    ret
add_matrices endp
